...
function readWorkspaceTabs(){try{const value=JSON.parse(window.sessionStorage.getItem("core-workspace-tabs")||"[]");return Array.isArray(value)?value.filter(item=>typeof item==="string"):[];}catch{return[];}}
function limitWorkspaceTabs(keys:string[],homeKey?:string){const unique=[...new Set(keys)];if(unique.length<=12)return unique;const rest=unique.filter(key=>key!==homeKey).slice(-11);return homeKey?[homeKey,...rest]:rest;}
...
export default function Home() {
  ...
  const [navigation,setNavigation]=useState<NavigationModel|null>(null);const [expandedSection,setExpandedSection]=useState("");const [expandedGroup,setExpandedGroup]=useState("");const [view,setView]=useState<View>("home");const[openTabKeys,setOpenTabKeys]=useState<string[]>([]);
  ...
  const selectRoute=(model:NavigationModel)=>{const pages=model.sections.flatMap(section=>section.items.filter(item=>item.type==="PAGE").map(item=>({section,item})));const requestedRoute=window.location.pathname;const route=LEGACY_MES_ROUTES[requestedRoute]??requestedRoute;const selected=pages.find(entry=>entry.item.route===route)||pages.find(entry=>entry.item.key==="core.home")||pages[0];if(selected){const home=pages.find(entry=>entry.item.key==="core.home")?.item;setView(selected.item.viewKey as View);setExpandedSection(selected.section.key);setExpandedGroup(selected.item.parentKey);setOpenTabKeys(previous=>{const stored=previous.length?previous:readWorkspaceTabs();const allowed=new Set(pages.map(entry=>entry.item.key));const tabs=[home?.key,...stored.filter(key=>allowed.has(key)),selected.item.key].filter((key):key is string=>Boolean(key));return limitWorkspaceTabs([...new Set(tabs)],home?.key);});if(requestedRoute!==selected.item.route)window.history.replaceState(null,"",selected.item.route);}};
  ...
  useEffect(()=>{if(navigation)window.sessionStorage.setItem("core-workspace-tabs",JSON.stringify(openTabKeys));},[navigation,openTabKeys]);
  ...
  const openTabEntries=openTabKeys.map(key=>allPages.find(entry=>entry.item.key===key)).filter((entry):entry is {section:NavigationSection;item:NavigationItem}=>Boolean(entry));
  ...
  const openItem=(item:NavigationItem)=>{if(item.type!=="PAGE")return;const entry=allPages.find(candidate=>candidate.item.key===item.key);const homeKey=allPages.find(candidate=>candidate.item.key==="core.home")?.item.key;setOpenTabKeys(previous=>limitWorkspaceTabs([...new Set([homeKey,...previous,item.key].filter((key):key is string=>Boolean(key)))],homeKey));setView(item.viewKey as View);if(entry)setExpandedSection(entry.section.key);setExpandedGroup(item.parentKey);setSidebarOpen(false);setCommandOpen(false);setCommandQuery("");if(window.location.pathname!==item.route||window.location.search)window.history.pushState(null,"",item.route);window.scrollTo({top:0,behavior:"smooth"});if(navigation){const recent=[item.key,...navigation.recentKeys.filter(key=>key!==item.key)].slice(0,10);void savePreferences(navigation.favoriteKeys,recent);}};
  const closeWorkspaceTab=(key:string)=>{const closingIndex=openTabKeys.indexOf(key);const closing=allPages.find(entry=>entry.item.key===key)?.item;if(!closing||closing.key==="core.home")return;const nextKeys=openTabKeys.filter(tabKey=>tabKey!==key);setOpenTabKeys(nextKeys);if(closing.viewKey!==view)return;const nextKey=nextKeys[Math.min(closingIndex,nextKeys.length-1)]||"core.home";const next=allPages.find(entry=>entry.item.key===nextKey)?.item;if(next)openItem(next);};
  ...
  const renderWorkspacePage=(entry:{section:NavigationSection;item:NavigationItem})=>{const tabView=entry.item.viewKey as View;return <section className="workspace-page" key={entry.item.key} hidden={tabView!==view} aria-hidden={tabView!==view}>
    {tabView==="home"&&businessSection&&(data?<Overview onNavigate={navigate} data={data} displayName={user?.displayName}/>:<BusinessHome section={businessSection} onOpen={openItem}/>)}
    {tabView==="approvals"&&<DemoApprovalWorkspace apiUrl={API_URL}/>}
    {tabView.startsWith("mes-")&&<MesWorkspace view={tabView as MesView} apiUrl={API_URL}/>}
    {data&&tabView==="modules"&&<Modules items={data.modules} onStatus={changeModuleStatus}/>}
    {data&&tabView==="resources"&&<Resources items={data.resources} onChanged={refresh}/>}
    {tabView==="users"&&<Users/>}
    {tabView==="organizations"&&<Organizations/>}
    {tabView==="access"&&<Access/>}
    {data&&tabView==="activity"&&<Activity items={data.activities}/>}
    {data&&tabView==="files"&&<Files items={data.files} storageGb={data.summary.storageGb} onUpload={uploadFile} onDownload={downloadFile}/>}
    {data&&tabView==="settings"&&<Settings values={data.settings} onSave={items=>mutate("/api/v1/control-plane/settings","PUT",items)}/>}
    {entry.section.key==="system-administration"&&!data&&<div className="auth-loading" aria-label="Đang tải dữ liệu quản trị"><span/></div>}
  </section>;};
  ...
  return <div className="app-shell">
    ...
    </header><nav className="workspace-tabs" aria-label="Các module đang mở">
      <div className="workspace-tabs-track">{openTabEntries.map(entry=><div className={`workspace-tab ${view===entry.item.viewKey?"active":""}`} key={entry.item.key}><button className="workspace-tab-select" aria-current={view===entry.item.viewKey?"page":undefined} onClick={()=>openItem(entry.item)}><AppIcon name={entry.item.icon} size={14}/><span>{entry.item.label}</span></button>{entry.item.key!=="core.home"&&<button className="workspace-tab-close" aria-label={`Đóng ${entry.item.label}`} onClick={()=>closeWorkspaceTab(entry.item.key)}><AppIcon name="x" size={12}/></button>}</div>)}</div><span className="workspace-tab-count">{openTabEntries.length}/12</span>
    </nav><main>
      {operationError&&<p className="auth-error" role="alert">{operationError}</p>}
      {openTabEntries.map(renderWorkspacePage)}
    </main></div>
