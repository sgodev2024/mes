...
type DashboardLink={tab?:string;demoRow?:number;focus?:string};
...
const navigate=(route:string,context:DashboardLink={})=>{const params=new URLSearchParams();params.set("from","executive-dashboard");if(context.tab)params.set("tab",context.tab);if(context.demoRow!==undefined)params.set("demoRow",String(context.demoRow));if(context.focus)params.set("focus",context.focus);const query=params.toString();window.history.pushState(null,"",`${route}${query?`?${query}`:""}`);window.dispatchEvent(new PopStateEvent("popstate"));window.scrollTo({top:0,behavior:"smooth"});};
    ...
    <section id="executive-tabpanel" role="tabpanel" aria-labelledby={`executive-tab-${activeDomain}`} className="mes-executive-tabpanel">
      {activeDomain==="01"?<>
        ...
        <section className="mes-executive-kpis">
          <ExecutiveKpi icon="factory" label="Sản lượng ngày" value={liveOverview?formatTonne(production,hasMetric(summary,"mined_receipt","processing_receipt")):"24.580 t"} note={liveOverview?`${summary?.releases||0} bản phát hành`:"+8,4% so với cùng kỳ"} route="/mes/production-operations" tab="Sản lượng ngày"/>
          <ExecutiveKpi icon="calendar" label="Tiến độ kế hoạch" value="86,7%" note="+5,2% so với kỳ trước" tone="green" route="/mes/planning-performance" tab="Kế hoạch"/>
          ...
          <ExecutiveKpi icon="flask" label="Chất lượng đạt" value="97,6%" note="125/128 mẫu đạt" tone="green" route="/mes/quality-acceptance" tab="Kết quả KCS"/>
          <ExecutiveKpi icon="truck" label="Tiêu thụ tháng" value={liveOverview?formatTonne(consumption,hasMetric(summary,"sale_issue")):"432.870 t"} note="86,6% kế hoạch" route="/mes/sales-logistics" tab="Đã bàn giao"/>
          <ExecutiveKpi icon="circle-alert" label="Cảnh báo mở" value={liveOverview?viNumber((recon?.variances||0)+(recon?.incomplete||0)+(recon?.unverifiedMasterData||0)):"12"} note="2 sự cố · 10 vượt ngưỡng" tone="red" route="/mes/alerts-directives" tab="Cần xử lý"/>
        </section>
        <section className="mes-executive-main-grid">
          <article className="panel mes-executive-chart mes-dashboard-drillable" role="button" tabIndex={0} onClick={()=>navigate("/mes/production-operations",{tab:"Sản lượng ngày",focus:"Nhịp sản xuất 14 ngày gần nhất"})} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")navigate("/mes/production-operations",{tab:"Sản lượng ngày",focus:"Nhịp sản xuất 14 ngày gần nhất"});}}><header><div><h3>Nhịp sản xuất 14 ngày gần nhất</h3><p>Sản lượng thực hiện theo ngày · bấm để mở dữ liệu sản xuất</p></div><span><i/>Sản lượng thực hiện</span></header><div className="mes-executive-bars">{displaySeries.map(point=><div key={point.date} title={`${formatDate(point.date)}: ${viNumber(point.value)} tấn`}><b style={{height:`${Math.max(8,point.value/maxSeries*100)}%`}}/><small>{new Date(`${point.date}T00:00:00`).getDate()}</small></div>)}</div></article>
          <article className="panel mes-executive-decisions"><header><div><h3>Quyết định cần ban hành</h3><p>Cảnh báo gần nhất, xếp theo mức độ ảnh hưởng vận hành</p></div><strong>{decisions.length} việc</strong></header>{decisions.map((item,index)=>{const target=liveOverview?{route:"/mes/coal-flow",tab:undefined}:index===0?{route:"/mes/occupational-safety",tab:"Nguy cơ"}:index===1?{route:"/mes/planning-performance",tab:"Dự kiến"}:{route:"/mes/sales-logistics",tab:"Đang giao"};return <button key={item.productId} onClick={()=>navigate(target.route,{tab:target.tab,focus:`${item.productCode} · ${item.productName}`})}><em className={index===0?"urgent":"high"}>{index===0?"KHẨN":"CAO"}</em><span><b>{item.productCode} · {item.productName}</b><small>{item.reason||`Chênh lệch ${viNumber(item.variance)} ${item.unitCode}`}</small></span><i>Xem chi tiết →</i></button>;})}</article>
          <article className="panel mes-executive-efficiency"><header><div><h3>Hiệu quả theo sản phẩm</h3><p>Mức cân bằng kế hoạch · sản xuất · tồn</p></div><button onClick={()=>navigate("/mes/coal-flow",{focus:"Hiệu quả theo sản phẩm"})}>Xem chi tiết</button></header>{efficiency.map((item,index)=><button type="button" className="mes-efficiency-row" key={item.productId} onClick={()=>navigate("/mes/coal-flow",{focus:item.productName||item.productCode})}><b>{index+1}</b><strong>{item.productName||item.productCode}</strong><span><i style={{width:`${item.score}%`}}/></span><em>{item.score}%</em><small className={item.score>=90?"green":item.score>=70?"amber":"red"}>{statusLabel(item.status)}</small></button>)}</article>
          <article className="panel mes-executive-risks"><header><div><h3>An toàn & rủi ro vận hành</h3><p>Điểm nóng cần theo dõi trong ngày</p></div><button onClick={()=>navigate("/mes/occupational-safety",{tab:"Tổng quan"})}>Mở module</button></header><div className="mes-executive-risk-kpis">{(safety?.kpis||[{label:"Ngày công an toàn",value:"128"},{label:"Nguy cơ đang mở",value:"16"},{label:"Đúng hạn",value:"91,2%"}]).slice(0,3).map((item,index)=><button type="button" key={item.label} onClick={()=>navigate("/mes/occupational-safety",{tab:index===0?"Tổng quan":index===1?"Nguy cơ":"Hành động khắc phục",focus:item.label})}><small>{item.label}</small><strong>{item.value}</strong></button>)}</div>{(safety?.attention||[{id:"s1",title:"Khí CH4 vượt ngưỡng",detail:"Khu vực XV-120 lúc 13:42",severity:"Cao"},{id:"s2",title:"Rào chắn chưa đạt yêu cầu",detail:"Tuyến vận tải số 3",severity:"Trung bình"}]).slice(0,4).map(item=><button className="mes-executive-risk-row" key={item.id} onClick={()=>navigate("/mes/occupational-safety",{tab:"Nguy cơ",focus:item.title})}><i className={item.severity==="Nghiêm trọng"||item.severity==="Cao"?"red":"amber"}/><span><strong>{item.title}</strong><small>{item.detail}</small></span><em>{item.severity}</em></button>)}</article>
        </section>
...
function DomainDashboard({domain,definition}:{domain:ExecutiveDomain;definition:MesPrototypeDefinition}){
  ...
  const attentionTab=domain.fixtureKey==="mes-occupational-safety"?"Nguy cơ":domain.fixtureKey==="mes-materials-equipment"?"Sửa chữa":domain.fixtureKey==="mes-finance-accounting"?"Công nợ":domain.fixtureKey==="mes-esg"?"Môi trường":definition.tabs[0];
  return <div className="mes-domain-dashboard">
    <div className="mes-dashboard-demo-banner"><AppIcon name={definition.icon} size={17}/><span><strong>{definition.title}</strong><small>{definition.description}</small></span><em>{definition.sourceMode}</em><button type="button" onClick={()=>navigate(domain.route,{tab:definition.tabs[0]})}>Mở module nghiệp vụ <AppIcon name="chevron-right" size={14}/></button></div>
    <section className="mes-domain-kpis">{definition.kpis.map((item,index)=><button type="button" className={`panel mes-domain-kpi ${item.tone}`} key={item.label} onClick={()=>navigate(domain.route,{tab:definition.tabs[Math.min(index,definition.tabs.length-1)],focus:item.label})}><span><AppIcon name={definition.icon} size={18}/></span><div><small>{item.label}</small><strong>{item.value}<i>{item.unit}</i></strong><em>{item.delta}</em></div></button>)}</section>
    <section className="mes-domain-grid">
      <article className="panel mes-domain-trend"><header><div><h3>{definition.trendLabel}</h3><p>Bấm từng kỳ để mở đúng lát cắt dữ liệu</p></div><span><i/>Thực hiện</span></header><div className="mes-domain-bars">{definition.trend.map((value,index)=><button type="button" key={`${definition.title}-${index}`} onClick={()=>navigate(domain.route,{tab:definition.tabs[0],focus:`${definition.trendLabel} · Kỳ ${index+1}`})}><strong>{String(value).replace(".",",")}</strong><b style={{height:`${Math.max(12,value/max*100)}%`}}/><small>Kỳ {index+1}</small></button>)}</div></article>
      <article className="panel mes-domain-attention"><header><div><h3>Công việc cần chú ý</h3><p>Ngoại lệ cần Ban lãnh đạo theo dõi</p></div><strong>{definition.attention.length} cảnh báo</strong></header>{definition.attention.map((item,index)=><button key={item.title} onClick={()=>navigate(domain.route,{tab:attentionTab,focus:item.title})}><span className={index===0?"red":"amber"}><AppIcon name="circle-alert" size={16}/></span><div><b>{item.title}</b><small>{item.detail}</small></div><em>{item.severity}</em></button>)}</article>
    </section>
    <article className="panel mes-domain-table"><header><div><h3>Dữ liệu điều hành gần nhất</h3><p>{definition.title} · bấm một dòng để xem đúng bản ghi demo</p></div><button type="button" onClick={()=>navigate(domain.route,{tab:definition.tabs[0]})}>Xem tất cả</button></header><div className="mes-table-scroll"><table><thead><tr><th>STT</th>{definition.columns.map(column=><th key={column.key} className={column.numeric?"numeric":""}>{column.label}</th>)}</tr></thead><tbody>{definition.rows.map((row,index)=><tr className="mes-dashboard-data-row" tabIndex={0} key={`${definition.title}-${index}`} onClick={()=>navigate(domain.route,{tab:definition.tabs[0],demoRow:index,focus:String(row[definition.columns[0].key]??definition.title)})} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")navigate(domain.route,{tab:definition.tabs[0],demoRow:index,focus:String(row[definition.columns[0].key]??definition.title)});}}><td>{index+1}</td>{definition.columns.map(column=><td key={column.key} className={column.numeric?"numeric":""}>{column.key==="status"?<span className={`mes-status ${demoStatusTone(String(row[column.key]??""))}`}>{String(row[column.key]??"—")}</span>:String(row[column.key]??"—")}</td>)}</tr>)}</tbody></table></div></article>
  </div>;
}
...
function ExecutiveKpi({icon,label,value,note,tone="blue",route,tab}:{icon:string;label:string;value:string;note:string;tone?:string;route:string;tab?:string}){return <button className={`panel mes-executive-kpi ${tone}`} onClick={()=>navigate(route,{tab,focus:label})}><span><AppIcon name={icon} size={18}/></span><small>{label}</small><strong>{value}</strong><em>{note}</em></button>}
