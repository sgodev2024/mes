...
public class SecurityConfig {
  ...
  @Bean CorsConfigurationSource corsConfigurationSource(
      @Value("${core.cors.allowed-origin-patterns}") String configuredOrigins){
    var allowedOrigins = Arrays.stream(configuredOrigins.split(","))
        .map(String::trim).filter(origin -> !origin.isBlank()).toList();
    if (allowedOrigins.isEmpty()) throw new IllegalStateException("CORS phải có ít nhất một origin hợp lệ");
